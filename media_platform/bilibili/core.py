...
class BilibiliCrawler(AbstractCrawler):
            ...
            for day in pd.date_range(start=config.START_DAY, end=config.END_DAY, freq="D"):
                ...
                while True:
                    ...
                    try:
                        ...
                        # Sleep after page navigation
                        sleep_seconds = await utils.crawler_sleep(config.CRAWLER_PAGE_SLEEP_SEC)
                        utils.logger.info(f"[BilibiliCrawler.search_by_keywords_in_time_range] Sleeping for {sleep_seconds:.2f} seconds after page {page-1}")
    ...
    async def get_creator_videos(self, creator_id: int):
        ...
        while True:
            ...
            if int(result["page"]["count"]) <= pn * ps:
                break
            sleep_seconds = await utils.crawler_sleep(config.CRAWLER_PAGE_SLEEP_SEC)
            utils.logger.info(f"[BilibiliCrawler.get_creator_videos] Sleeping for {sleep_seconds:.2f} seconds after page {pn}")
            pn += 1
    ...
    async def get_video_info_task(self, aid: int, bvid: str, semaphore: asyncio.Semaphore) -> Optional[Dict]:
        ...
        async with semaphore:
            try:
                ...
                utils.logger.info(f"[BilibiliCrawler.get_video_info_task] Sleeping for {sleep_seconds:.2f} seconds after fetching video details {bvid or aid}")
    ...
    async def get_bilibili_video(self, video_item: Dict, semaphore: asyncio.Semaphore):
        ...
        sleep_seconds = await utils.crawler_sleep(config.CRAWLER_DETAIL_SLEEP_SEC)
        utils.logger.info(f"[BilibiliCrawler.get_bilibili_video] Sleeping for {sleep_seconds:.2f} seconds after fetching video {aid}")
        if content is None:
            return
